...
pub struct Config {
    ...
    /// Comma-separated list of worker capabilities/tags (e.g., "gpu,high-memory")
...
/// Validate worker tag format
///
/// # Errors
///
/// Returns an error if the tag is invalid
pub fn validate_worker_tag(tag: &str) -> anyhow::Result<()> {
    if tag.is_empty() {
        anyhow::bail!("Worker tag cannot be empty");
    }

    if tag.len() > 32 {
        anyhow::bail!("Worker tag cannot exceed 32 characters");
    }

    // Check for control characters
    if tag.chars().any(char::is_control) {
        anyhow::bail!("Worker tag contains invalid characters");
    }
    ...
    {
        anyhow::bail!(
            "Worker tag can only contain alphanumeric characters, hyphens, and underscores"
        );
    }
