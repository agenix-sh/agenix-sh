...
impl RespClient {
    ...
    /// Register worker's capabilities/tags with AGQ
    ///
    /// Stores the tag list in the `worker:<id>:tags` key as a comma-separated string.
    ///
    /// # Errors
    ///
    /// Returns an error if the RESP protocol command fails
    pub async fn register_tags(&mut self, worker_id: &str, tags: &[String]) -> AgwResult<()> {
        if tags.is_empty() {
            return Ok(());
        }

        let key = format!("worker:{worker_id}:tags");
        let value = tags.join(",");

        info!(
            "Registering {} tags for worker {worker_id}: {value}",
            tags.len()
        );

        self.set(&key, &value).await?;

        info!("Successfully registered tags for worker {worker_id}");
        Ok(())
    }
    ...
    pub async fn job_get(&mut self, job_id: &str) -> AgwResult<String> {
        ...
        // Use JOB.GET command (not raw GET - jobs are stored as hashes)
    ...
    pub async fn plan_get(&mut self, plan_id: &str) -> AgwResult<String> {
        ...
        // Use PLAN.GET command (not raw GET - plans are stored as hashes)
        let json: String = Cmd::new()
            .arg("PLAN.GET")
            .arg(plan_id)
            .query_async(&mut self.connection)
