...
///
/// A Job now represents a single Task execution instance.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct Job {
    /// Unique job identifier
    pub id: String,

    /// Action ID that triggered this job
    pub action_id: String,

    /// Reference to the plan
    pub plan_id: String,

    /// Task number within the plan
    pub task_number: u32,
...
impl Job {
    ...
    /// # Errors
    ///
    /// Returns an error if fields contain invalid characters or exceed limits
    pub fn validate(&self) -> AgwResult<()> {
        // Validate IDs
        validate_string_field(&self.id, "id", MAX_JOB_ID_LEN, true)?;
        validate_string_field(&self.action_id, "action_id", MAX_JOB_ID_LEN, true)?;
