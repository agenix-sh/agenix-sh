...
pub(crate) fn get_current_timestamp_secs() -> Result<u64> {
...
fn handle_action_submit(args: &[RespValue], db: &Database) -> Result<RespValue> {
    ...
    for (_idx, input) in inputs.iter().enumerate() {
        ...
        // We need to clone input_jobs to modify them, which is tricky with the loop
        // Instead, let's build a map of dependents
        let mut dependents_map: HashMap<String, HashSet<String>> = HashMap::new();
        for job in &input_jobs {
            for dep_id in &job.dependencies {
                dependents_map.entry(dep_id.clone()).or_default().insert(job.id.clone());
            }
        }

        for job in &mut input_jobs {
            if let Some(deps) = dependents_map.get(&job.id) {
                job.dependents = deps.clone();
            }
        }

        all_jobs.extend(input_jobs);
    }

    // Submit jobs via Orchestrator
    let orchestrator = Orchestrator::new(db);
    orchestrator.submit_jobs(all_jobs)?;

    // Store Action metadata
    db.hset(&action_key, "plan_id", plan_id.as_bytes())?;
    db.hset(&action_key, "jobs_created", job_ids.len().to_string().as_bytes())?;
    ...
    db.hset(&action_key, "jobs_pending", job_ids.len().to_string().as_bytes())?;
